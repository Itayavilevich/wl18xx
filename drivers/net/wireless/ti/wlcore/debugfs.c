...
static ssize_t change_bw_write(struct file *file,
			       const char __user *user_buf,
			       size_t count, loff_t *ppos)
{
	struct wl1271 *wl = file->private_data;
	...
	wl12xx_for_each_wlvif(wl, wlvif) {
		wlcore_acx_peer_ht_operation_mode(wl, wlvif->sta.hlid, !!value);
	}
...
static const struct file_operations change_bw_ops = {
	.write = change_bw_write,
	.open = simple_open,
	.llseek = default_llseek,
};
